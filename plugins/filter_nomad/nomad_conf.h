...
#ifndef FLB_FILTER_NOMAD_CONF_H
#define FLB_FILTER_NOMAD_CONF_H
...
/* Nomad API server info */
#define FLB_API_HOST  "127.0.0.1"
#define FLB_API_PORT  4646
#define FLB_API_TLS   FLB_TRUE

/* Kubelet info */
#define FLB_NOMAD_HOST  "127.0.0.1"
 ...
 */
#define FLB_NOMAD_TAG_PREFIX "var.lib.nomad.alloc."
...
struct flb_nomad {
    ...
    /* Configuration proposed through JobMeta (boolean) */
    int nomad_logging_parser;   /* allow to process a suggested parser ? */
    int nomad_logging_exclude;  /* allowed to suggest to exclude logs ?  */
    ...
    /* Job Name where Fluent Bit is running */
    char *jobname;
    size_t jobname_len;

    /* Nomad Token from FLB_NOMAD_TOKEN file */
    char *token_file;
    char *token;
    size_t token_len;
    /* Command to get Nomad ACL Token */
    const char *nomad_token_command; 
    int nomad_token_create;
...
struct flb_nomad *flb_nomad_conf_create(struct flb_filter_instance *i,
                                      struct flb_config *config);
void flb_nomad_conf_destroy(struct flb_nomad *ctx);
