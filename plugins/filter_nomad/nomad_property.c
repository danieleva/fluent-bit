...
#define FLB_NOMAD_PROP_PARSER "parser"
#define FLB_NOMAD_PROP_PARSER_LEN (sizeof(FLB_NOMAD_PROP_PARSER) - 1)
#define FLB_NOMAD_PROP_EXCLUDE "exclude"
#define FLB_NOMAD_PROP_EXCLUDE_LEN (sizeof(FLB_NOMAD_PROP_EXCLUDE) - 1)
...
static inline void prop_not_allowed(const char *prop, struct flb_nomad_meta *meta,
                                    struct flb_nomad *ctx)
{
    flb_plg_warn(ctx->ins, "annotation '%s' not allowed "
                 "(ns='%s' group_name='%s')",
                 prop, meta->namespace, meta->jobname);
}

/* Property: parser */
static int prop_set_parser(struct flb_nomad *ctx, struct flb_nomad_meta *meta,
                           int is_task_specific, int stream,
                           ...
                           struct flb_nomad_props *props)
{
    ...
    if (ctx->nomad_logging_parser == FLB_FALSE) {
    ...
    if (!parser) {
        flb_plg_warn(ctx->ins, "annotation parser '%s' not found "
                     "(ns='%s' pod_name='%s', task_name='%s')",
                     tmp, meta->namespace, meta->jobname, meta->task_name);
        flb_free(tmp);
        return -1;
    }

    /* Save the parser in the properties context */
    if ((stream == FLB_NOMAD_PROP_NO_STREAM ||
         stream == FLB_NOMAD_PROP_STREAM_STDOUT) &&
        (is_task_specific == FLB_TRUE ||
         props->stdout_parser == FLB_NOMAD_PROP_UNDEF)) {
        props->stdout_parser = flb_sds_create(tmp);
    }
    if ((stream == FLB_NOMAD_PROP_NO_STREAM ||
         stream == FLB_NOMAD_PROP_STREAM_STDERR) &&
        (is_task_specific == FLB_TRUE ||
         props->stderr_parser == FLB_NOMAD_PROP_UNDEF)) {
...
static int prop_set_exclude(struct flb_nomad *ctx, struct flb_nomad_meta *meta,
                            int is_task_specific, int stream,
                            const char *val_buf, size_t val_len,
                            struct flb_nomad_props *props)
{
    ...
    if (ctx->nomad_logging_exclude == FLB_FALSE) {
    ...
    exclude = flb_utils_bool(tmp) == FLB_TRUE ?
              FLB_NOMAD_PROP_TRUE : FLB_NOMAD_PROP_FALSE;

    /* Save the exclude property in the context */
    if ((stream == FLB_NOMAD_PROP_NO_STREAM ||
         stream == FLB_NOMAD_PROP_STREAM_STDOUT) &&
        (is_task_specific == FLB_TRUE ||
         props->stdout_exclude == FLB_NOMAD_PROP_UNDEF)) {
        props->stdout_exclude = exclude;
    }
    if ((stream == FLB_NOMAD_PROP_NO_STREAM ||
         stream == FLB_NOMAD_PROP_STREAM_STDERR) &&
        (is_task_specific == FLB_TRUE ||
         props->stderr_exclude == FLB_NOMAD_PROP_UNDEF)) {
...
int flb_nomad_prop_set(struct flb_nomad *ctx, struct flb_nomad_meta *meta,
                      ...
                      struct flb_nomad_props *props)
{
    ...
    size_t task_len = 0;
    int stream = FLB_NOMAD_PROP_NO_STREAM;
    int (*function)(struct flb_nomad *ctx, struct flb_nomad_meta *meta,
                    int is_task_specific, int stream,
                    const char *val_buf, size_t val_len,
                    struct flb_nomad_props *props);

    if (prop_cmp(FLB_NOMAD_PROP_PARSER, FLB_NOMAD_PROP_PARSER_LEN, prop, prop_len)) {
        function = prop_set_parser;
        cur += FLB_NOMAD_PROP_PARSER_LEN;
    }
    else if (prop_cmp(FLB_NOMAD_PROP_EXCLUDE, FLB_NOMAD_PROP_EXCLUDE_LEN, prop, prop_len)) {
        function = prop_set_exclude;
        cur += FLB_NOMAD_PROP_EXCLUDE_LEN;
    }
    else {
        flb_plg_warn(ctx->ins, "unknown annotation 'fluentbit.io/%.*s' "
                     "(ns='%s' task_name='%s')",
                     prop_len, prop, meta->namespace, meta->jobname);
        return -1;
    }
    ...
    if (prop_cmp("_", 1, cur, len)) {
        ...
        if (prop_cmp("stdout", sizeof("stdout") - 1, cur, len)) {
            ...
            flb_plg_warn(ctx->ins, "invalid stream in annotation "
                         "'fluentbit.io/%.*s' (ns='%s' group_name='%s')",
                      prop_len, prop, meta->namespace, meta->jobname);
            return -1;
        }
     ...
     */
    if (task && strncmp(task, meta->task_name, task_len)) {
        return 0;
    }

    return function(ctx, meta,
                    (task ? FLB_TRUE : FLB_FALSE), stream,
                    val_buf, val_len, props);
}

int flb_nomad_prop_pack(struct flb_nomad_props *props,
                       void **out_buf, size_t *out_size)
{
    ...
    size = FLB_NOMAD_NUMBER_OF_PROPS;
    ...
    /* Index 0: FLB_NOMAD_PROPS_STDOUT_PARSER */
    ...
    /* Index 1: FLB_NOMAD_PROPS_STDERR_PARSER */
    ...
    /* Index 2: FLB_NOMAD_PROPS_STDOUT_EXCLUDE */
    if (props->stdout_exclude == FLB_NOMAD_PROP_TRUE) {
    ...
    /* Index 3: FLB_NOMAD_PROPS_STDERR_EXCLUDE */
    if (props->stderr_exclude == FLB_NOMAD_PROP_TRUE) {
        msgpack_pack_true(&pck);
    }
    else {
        msgpack_pack_false(&pck);
...
int flb_nomad_prop_unpack(struct flb_nomad_props *props,
                         const char *buf, size_t size)
{
    ...
    memset(props, '\0', sizeof(struct flb_nomad_props));
    ...
    o = root.via.array.ptr[FLB_NOMAD_PROPS_STDOUT_PARSER];
    ...
    o = root.via.array.ptr[FLB_NOMAD_PROPS_STDERR_PARSER];
    if (o.type == MSGPACK_OBJECT_NIL) {
    ...
    o = root.via.array.ptr[FLB_NOMAD_PROPS_STDOUT_EXCLUDE];
    props->stdout_exclude = o.via.boolean;

    /* Index 3: stderr_exclude */
    o = root.via.array.ptr[FLB_NOMAD_PROPS_STDERR_EXCLUDE];
...
void flb_nomad_prop_destroy(struct flb_nomad_props *props)
{
    if (props->stdout_parser) {
