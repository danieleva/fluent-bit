...
struct flb_nomad *flb_nomad_conf_create(struct flb_filter_instance *ins,
                                      struct flb_config *config)
{
    ...
    ctx = flb_calloc(1, sizeof(struct flb_nomad));
    if (!ctx) {
        flb_errno();
        return NULL;
    }
    ...
    /* Nomad Token Command */
    cmd = flb_filter_get_property("nomad_token_command", ins);
    if (cmd) {
        ctx->nomad_token_command = cmd;
    }
    else {
        ctx->nomad_token_command = NULL;
    }
    ...
    /* Get Nomad API endpoint */
    url = flb_filter_get_property("nomad_url", ins);
    ...
    snprintf(ctx->nomad_url, sizeof(ctx->nomad_url) - 1,
             "%s://%s:%i",
             ctx->api_https ? "https" : "http",
             ctx->api_host, ctx->api_port);

    if (ctx->nomad_meta_cache_ttl > 0) {
        ctx->hash_table = flb_hash_create_with_ttl(ctx->nomad_meta_cache_ttl,
