    ...
    for (i = 0; i < map.size; i++) {
        ...
        if (k.type == MSGPACK_OBJECT_STR &&
            ...
            if (strncmp(v.via.str.ptr, "stdout", v.via.str.size) == 0) {
                return FLB_NOMAD_PROP_STREAM_STDOUT;
            }
            else if (strncmp(v.via.str.ptr, "stderr", v.via.str.size) == 0) {
                return FLB_NOMAD_PROP_STREAM_STDERR;
            }
            else {
               return FLB_NOMAD_PROP_STREAM_UNKNOWN;
            }
        }
    }

    return FLB_NOMAD_PROP_NO_STREAM;
}
...
static int cb_nomad_init(struct flb_filter_instance *f_ins,
                        ...
                        void *data)
{
    ...
    ctx = flb_nomad_conf_create(f_ins, config);
    if (!ctx) {
        return -1;
    }

    /* Initialize regex context */
    ret = flb_nomad_regex_init(ctx);
    if (ret == -1) {
        flb_nomad_conf_destroy(ctx);
        return -1;
    }
     ...
     */
    flb_nomad_meta_init(ctx, config);
...
static int pack_map_content(msgpack_packer *pck, msgpack_sbuffer *sbuf,
                            ...
                            const char *nomad_buf, size_t nomad_size,
                            ...
                            struct flb_nomad *ctx)
{
    ...
    if (nomad_buf && nomad_size > 0) {
        ...
        msgpack_pack_str_body(pck, "nomad", 10);

        off = 0;
        msgpack_unpacked_init(&result);
        msgpack_unpack_next(&result, nomad_buf, nomad_size, &off);
...
static int cb_nomad_filter(const void *data, size_t bytes,
                          ...
                          struct flb_config *config)
{
    ...
    struct flb_nomad *ctx = filter_context;
    struct flb_nomad_meta meta = {0};
    ...
    while (msgpack_unpack_next(&result, data, bytes, &off) == MSGPACK_UNPACK_SUCCESS) {
        ...
        case FLB_NOMAD_PROP_STREAM_STDOUT:
            {
                if (props.stdout_exclude == FLB_TRUE) {
                    /* Skip this record */
                    if (ctx->use_journal == FLB_TRUE) {
                        flb_nomad_meta_release(&meta);
            ...
            break;
        case FLB_NOMAD_PROP_STREAM_STDERR:
            {
...
static int cb_nomad_exit(void *data, struct flb_config *config)
{
    struct flb_nomad *ctx;
...
static struct flb_config_map config_map[] = {
    ...
    /* Buffer size for HTTP Client when reading responses from API Server */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, buffer_size),
    ...
    /* TLS: set debug 'level' */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, tls_debug),
    ...
    /* TLS: enable verification */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, tls_verify),
    ...
    /* TLS: set tls.vhost feature */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, tls_vhost),
    ...
    /* Merge structured record as independent keys */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, merge_log),
    ...
    /* New key name to merge the structured content of 'log' */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, merge_log_key),
    ...
    /* On merge, trim field values (remove possible ending \n or \r) */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, merge_log_trim),
    ...
    /* Keep original log key after successful merging/parsing */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, keep_log),
     "keep original log content if it was successfully parsed and merged"
    },

    /* Full Kubernetes API server URL */
    {
     FLB_CONFIG_MAP_STR, "nomad_url", "http://localhost:4646",
     0, FLB_FALSE, 0,
     "Nomad API server URL"
    },
     ...
     */
    {
     FLB_CONFIG_MAP_STR, "nomad_meta_preload_cache_dir", NULL,
     0, FLB_TRUE, offsetof(struct flb_nomad, meta_preload_cache_dir),
     "set directory with metadata files"
    },

    /* Kubernetes TLS: CA file */
    {
     FLB_CONFIG_MAP_STR, "nomad_ca_file", FLB_NOMAD_CA,
     0, FLB_TRUE, offsetof(struct flb_nomad, tls_ca_file),
     "Kubernetes TLS CA file"
    },

    /* Kubernetes TLS: CA certs path */
    {
     FLB_CONFIG_MAP_STR, "nomad_ca_path", NULL,
     0, FLB_TRUE, offsetof(struct flb_nomad, tls_ca_path),
     "Kubernetes TLS ca path"
    },

    /* Kubernetes Tag prefix */
    {
     FLB_CONFIG_MAP_STR, "nomad_tag_prefix", FLB_NOMAD_TAG_PREFIX,
     0, FLB_TRUE, offsetof(struct flb_nomad, nomad_tag_prefix),
     "prefix used in tag by the input plugin"
    },

    /* Nomad Token file */
    {
    //  FLB_CONFIG_MAP_STR, "nomad_token_file", FLB_NOMAD_TOKEN,
    FLB_CONFIG_MAP_STR, "nomad_token_file", NULL,
     0, FLB_TRUE, offsetof(struct flb_nomad, token_file),
     "Nomad ACL  token file"
    },

    /* Kubernetes Token command */
    {
     FLB_CONFIG_MAP_STR, "nomad_token_command", NULL,
     0, FLB_FALSE, 0,
    ...
    /* Include Kubernetes Labels in the final record ? */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, labels),
    ...
    /* Include Kubernetes Annotations in the final record ? */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, annotations),
     ...
     */
    {
     FLB_CONFIG_MAP_BOOL, "nomad-logging.parser", "false",
     0, FLB_TRUE, offsetof(struct flb_nomad, nomad_logging_parser),
     "allow Pods to suggest a parser"
    },
    {
     FLB_CONFIG_MAP_BOOL, "k8s-logging.exclude", "false",
     0, FLB_TRUE, offsetof(struct flb_nomad, nomad_logging_exclude),
    ...
    /* Use Systemd Journal mode ? */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, use_journal),
    ...
    /* Generate dummy metadata (only for test/dev purposes) */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, dummy_meta),
     ...
     */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, dns_retries),
     "dns lookup retries N times until the network start working"
    },

    {
     FLB_CONFIG_MAP_TIME, "dns_wait_time", "30",
     0, FLB_TRUE, offsetof(struct flb_nomad, dns_wait_time),
    ...
    /* Fetch K8s meta when docker_id has changed */
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_nomad, cache_use_docker_id),
     "fetch K8s meta when docker_id is changed"
    },

    {
     FLB_CONFIG_MAP_BOOL, "use_tag_for_meta", "false",
     0, FLB_TRUE, offsetof(struct flb_nomad, use_tag_for_meta),
     "use tag associated to retrieve metadata instead of nomad-server"
    },
 ...
 //    0, FLB_TRUE, offsetof(struct flb_nomad, use_kubelet),
 ...
 //   },
     ...
     */
 //   {
 //    FLB_CONFIG_MAP_INT, "nomad_port", "4646",
 //    0, FLB_TRUE, offsetof(struct flb_nomad, kubelet_port),
 ...
 //   },
     ...
     */
    {
     FLB_CONFIG_MAP_TIME, "nomad_meta_cache_ttl", "0",
     0, FLB_TRUE, offsetof(struct flb_nomad, nomad_meta_cache_ttl),
...
struct flb_filter_plugin filter_nomad_plugin = {
    .name         = "nomad",
    .description  = "Filter to append Nomad metadata",
    .cb_init      = cb_nomad_init,
    .cb_filter    = cb_nomad_filter,
    .cb_exit      = cb_nomad_exit,
