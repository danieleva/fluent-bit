...
#include "nomad_meta.h"
#include "nomad_property.h"

#define FLB_NOMAD_META_CONTAINER_STATUSES_KEY "containerStatuses"
#define FLB_NOMAD_META_CONTAINER_STATUSES_KEY_LEN \
    (sizeof(FLB_NOMAD_META_CONTAINER_STATUSES_KEY) - 1)
#define FLB_NOMAD_META_INIT_CONTAINER_STATUSES_KEY "initContainerStatuses"
#define FLB_NOMAD_META_INIT_CONTAINER_STATUSES_KEY_LEN \
    (sizeof(FLB_NOMAD_META_INIT_CONTAINER_STATUSES_KEY) - 1)
#define FLB_NOMAD_TOKEN_BUF_SIZE 8192       /* 8KB */
#define FLB_NOMAD_TOKEN_TTL 600             /* 10 minutes */
                                  ...
                                  char **out_buf, size_t *out_size)
{
    ...
    res = flb_calloc(1, FLB_NOMAD_TOKEN_BUF_SIZE);
    if (!res) {
        flb_errno();
        pclose(fp);
        return -1;
    ...
    while (fgets(buf, sizeof(buf), fp) != NULL) {
        len = strlen(buf);
        if (len >= FLB_NOMAD_TOKEN_BUF_SIZE - 1) {
            temp = flb_realloc(res, (FLB_NOMAD_TOKEN_BUF_SIZE + size) * 2);
            if (temp == NULL) {
                flb_errno();
                flb_free(res);
                pclose(fp);
...
/* Set Nomad Authorization Token and get HTTP Auth Header */
static int get_http_auth_header(struct flb_nomad *ctx) 
{
    int ret;
    char *temp;
    char *tk = NULL;
    ...
    if (ctx->nomad_token_command != NULL) {
#ifdef FLB_HAVE_NOMAD_TOKEN_COMMAND
        ret = get_token_with_command(ctx->nomad_token_command, &tk, &tk_size);
#else
        ret = -1;
#endif
        if (ret == -1) {
            flb_plg_warn(ctx->ins, "failed to run command %s", ctx->nomad_token_command);
        }
        ctx->nomad_token_create = time(NULL);
    } 
    else {
        if (ctx->token_file != NULL) {
        ret = file_to_buffer(ctx->token_file, &tk, &tk_size);
        if (ret == -1) {
            flb_plg_warn(ctx->ins, "cannot open %s", FLB_NOMAD_TOKEN);
        }
        /* Token from token file will not expire */
        /* Set the creation time to 0 to aviod refresh */
        ctx->nomad_token_create = 0;
        }
    }
...
static int refresh_token_if_needed(struct flb_nomad *ctx)
{
    int expired = 0;
    int ret;

    if (ctx->nomad_token_command != NULL) {
        if (ctx->nomad_token_create > 0) {
            if (time(NULL) > ctx->nomad_token_create + FLB_NOMAD_TOKEN_TTL) {
                expired = FLB_TRUE;
            }
        }
        
        if (expired || ctx->nomad_token_create == 0) {
...
static void expose_nomad_meta(struct flb_nomad *ctx)
{
    char *tmp;
    struct flb_env *env;
    ...
    flb_env_set(env, "nomad", "enabled");
  //  flb_env_set(env, "nomad.namespace", ctx->namespace);
  //  flb_env_set(env, "nomad.job_name", ctx->jobname);

    tmp = (char *) flb_env_get(env, "NODE_NAME");
    if (tmp) {
        flb_env_set(env, "nomad.node_name", tmp);
    }
}

/* Load local information from a POD context */
static int get_local_pod_info(struct flb_nomad *ctx)
{
    int ret;
    char *ns;
    size_t ns_size;
 ...
 //   if (ret == -1) {
  ...
  //      return FLB_FALSE;
   ...
   // ctx->namespace_len = ns_size;
    ...
    if (hostname) {
        ctx->jobname = flb_strdup(hostname);
        ctx->jobname_len = strlen(ctx->jobname);
    }
    else {
        char tmp[256];
        gethostname(tmp, 256);
        ctx->jobname = flb_strdup(tmp);
        ctx->jobname_len = strlen(ctx->jobname);
    }
    ...
    expose_nomad_meta(ctx);
    return FLB_TRUE;
}

/*
 * If a file exists called namespace_jobname.meta, load it and use it.
 * If not, fall back to API. This is primarily for diagnostic purposes,
 * e.g. debugging new parsers.
 */
static int get_meta_file_info(struct flb_nomad *ctx, const char *namespace,
                              const char *jobname, char **buffer, size_t *size,
                              int *root_type) {
    ...
    if (ctx->meta_preload_cache_dir && namespace && jobname) {

        ret = snprintf(uri, sizeof(uri) - 1, "%s/%s_%s.meta",
                       ctx->meta_preload_cache_dir, namespace, jobname);
        if (ret > 0) {
 ...
 * this could send out HTTP Request either to NOMAD API
 */
static int get_meta_info_from_request(struct flb_nomad *ctx,
                                      ...
                                      char* uri)
{
    ...
    if (!u_conn) {
        flb_plg_error(ctx->ins, "nomad upstream connection error");
        return -1;
    }
    ...
    if (ctx->auth_len > 0) {
        flb_http_add_header(c, "X-Nomad-Token", 13, ctx->auth, ctx->auth_len);
    }

    ret = flb_http_do(c, &b_sent);
    flb_plg_debug(ctx->ins, "Request (ns=%s, pod=%s) http_do=%i, "
                  "HTTP Status: %i",
                  namespace, jobname, ret, c->resp.status);
...
static int get_jobs_from_nomad(struct flb_nomad *ctx,
                                 const char *namespace, const char *jobname,
                                 char **out_buf, size_t *out_size)
{
    int ret;
    int packed = -1;
    ...
    *out_size = 0;
    ...
    packed = get_meta_file_info(ctx, namespace, jobname, &buf, &size,
                                &root_type);

    if (packed == -1) {

        ret = snprintf(uri, sizeof(uri) - 1, FLB_NOMAD_ALLOCS);
        if (ret == -1) {
            return -1;
        }
        flb_plg_debug(ctx->ins,
                      "Send out request to Nomad for allocs information.");
        packed = get_meta_info_from_request(ctx, namespace, jobname,
...
static int get_api_server_info(struct flb_nomad *ctx,
                               const char *namespace, const char *jobname,
                               char **out_buf, size_t *out_size)
{
    int ret;
...
//return 0;
    /* used for unit test purposes*/
    packed = get_meta_file_info(ctx, namespace, jobname,
                                &buf, &size, &root_type);

    if (packed == -1) {

        ret = snprintf(uri, sizeof(uri) - 1, FLB_NOMAD_HEALTH,
                       jobname);
        ...
        flb_plg_debug(ctx->ins,
                      "Send out request to API Server for jobs information");
        packed = get_meta_info_from_request(ctx, namespace, jobname,
                                            &buf, &size, &root_type, uri);
    }
...
{
    struct flb_nomad_meta *meta = data;

    if (vlen == 0) {
        return;
    }

    if (meta->jobname == NULL && strcmp(name, "group_name") == 0) {
        meta->jobname = flb_strndup(value, vlen);
        meta->jobname_len = vlen;
        meta->fields++;
    }
    ...
    else if (meta->task_name == NULL &&
             strcmp(name, "task_name") == 0) {
        meta->task_name = flb_strndup(value, vlen);
        meta->task_name_len = vlen;
        meta->fields++;
    }
...
*/
static void extract_container_hash(struct flb_nomad_meta *meta,
                                   msgpack_object status)
{
    ...
    for (i = 0;
        ...
        if ((k.via.str.size == FLB_NOMAD_META_CONTAINER_STATUSES_KEY_LEN &&
             strncmp(k.via.str.ptr,
                     FLB_NOMAD_META_CONTAINER_STATUSES_KEY,
                     FLB_NOMAD_META_CONTAINER_STATUSES_KEY_LEN) == 0) ||
            (k.via.str.size == FLB_NOMAD_META_INIT_CONTAINER_STATUSES_KEY_LEN &&
             strncmp(k.via.str.ptr,
                     FLB_NOMAD_META_INIT_CONTAINER_STATUSES_KEY,
                     FLB_NOMAD_META_INIT_CONTAINER_STATUSES_KEY_LEN) == 0)) {
            int j;
...
static int search_jobname_and_namespace(struct flb_nomad_meta *meta,
                                        struct flb_nomad *ctx,
                                        msgpack_object map)
{
    int i;
    int jobname_found = FLB_FALSE;
    int namespace_found = FLB_FALSE;
    int target_jobname_found = FLB_FALSE;
    int target_namespace_found = FLB_FALSE;

    msgpack_object k;
    msgpack_object v;

    for (i = 0; (!jobname_found || !namespace_found) &&
        ...
        if (k.via.str.size == 4 && !strncmp(k.via.str.ptr, "name", 4)) {
            ...
            jobname_found = FLB_TRUE;
            if (!strncmp(v.via.str.ptr, meta->jobname, meta->jobname_len)) {
                target_jobname_found = FLB_TRUE;
            }
    ...
    if (!target_jobname_found || !target_namespace_found) {
        return -1;
    }
...
static int search_metadata_in_items(struct flb_nomad_meta *meta,
                                    struct flb_nomad *ctx,
                                    ...
                                    msgpack_object *target_item_map)
{
    ...
    for (i = 0; !target_found && i < items_array.via.array.size; i++) {
        ...
        for (j = 0; j < item_info_map.via.map.size; j++) {
            ...
            if (k.via.str.size == 8 &&
                ...
                if (search_jobname_and_namespace(meta, ctx, v) == 0) {
                    target_found = FLB_TRUE;
                    *target_item_map = item_info_map;
                    flb_plg_debug(ctx->ins,
                                  "Nomad find pod: %s and ns: %s match",
                                            meta->jobname, meta->namespace);
                }
                break;
            }
        }
    ...
    if (!target_found) {
        flb_plg_debug(ctx->ins,
                      "nomad didn't find pod: %s, ns: %s match",
                      meta->jobname, meta->namespace);
        return -1;
    }
    return 0;
 ...
 *
 */
static int search_item_in_items(struct flb_nomad_meta *meta,
                                struct flb_nomad *ctx,
...
static int merge_meta_from_tag(struct flb_nomad *ctx, struct flb_nomad_meta *meta,
                               char **out_buf, size_t *out_size)
{
...
flb_plg_warn(ctx->ins, "mp_map %s", mp_pck);
    if (meta->jobname != NULL) {
        flb_mp_map_header_append(&mh);
        msgpack_pack_str(&mp_pck, 8);
        msgpack_pack_str_body(&mp_pck, "pod_name", 8);
        msgpack_pack_str(&mp_pck, meta->jobname_len);
        msgpack_pack_str_body(&mp_pck, meta->jobname, meta->jobname_len);
    }
    ...
    if (meta->task_name != NULL) {
        flb_mp_map_header_append(&mh);
        msgpack_pack_str(&mp_pck, 14);
        msgpack_pack_str_body(&mp_pck, "task_name", 9);
        msgpack_pack_str(&mp_pck, meta->task_name_len);
        msgpack_pack_str_body(&mp_pck, meta->task_name,
                              meta->task_name_len);
    }
...
static int merge_meta(struct flb_nomad_meta *meta, struct flb_nomad *ctx,
                      ...
                      char **out_buf, size_t *out_size)
{
    ...
    struct flb_nomad_props props = {0};
    ...
    if (api_buf != NULL) {
        ...
        if (ret == MSGPACK_UNPACK_SUCCESS) {
            ...
            for (i = 0; target_found && !(meta_found && spec_found && status_found) &&
                ...
                k = api_map.via.map.ptr[i].key;
                if (k.via.str.size == 8 && !strncmp(k.via.str.ptr, "Job", 8)) {
    ...
    if (meta->task_name != NULL) {
        msgpack_pack_str(&mp_pck, 14);
        msgpack_pack_str_body(&mp_pck, "task_name", 14);
        msgpack_pack_str(&mp_pck, meta->task_name_len);
        msgpack_pack_str_body(&mp_pck, meta->task_name,
                              meta->task_name_len);
    }
    ...
    if (have_annotations >= 0) {
        ...
        void *prop_buf;
        size_t prop_size;
        flb_nomad_prop_pack(&props, &prop_buf, &prop_size);
        msgpack_sbuffer_write(&mp_sbuf, prop_buf, prop_size);
        flb_nomad_prop_destroy(&props);
        flb_free(prop_buf);
    }
...
static inline int extract_meta(struct flb_nomad *ctx,
                               ...
                               struct flb_nomad_meta *meta)
{
    ...
    /* Reset meta context */
    memset(meta, '\0', sizeof(struct flb_nomad_meta));
    ...
    if (ctx->use_journal == FLB_TRUE) {
        ...
        while (msgpack_unpack_next(&mp_result, data, data_size, &off) == MSGPACK_UNPACK_SUCCESS) {
            ...
            /* Lookup the TASK_NAME key/value */
            ...
            for (i = 0; i < map.via.map.size; i++) {
                ...
                if (strncmp(key.via.str.ptr, "TASK_NAME", 14) == 0) {
         ...
         */
        nomad_tag_len = flb_sds_len(ctx->nomad_tag_prefix);
        if (nomad_tag_len + 1 >= tag_len) {
            flb_plg_error(ctx->ins, "incoming record tag (%s) is shorter "
                          "than nomad_tag_prefix value (%s), skip filter",
                          tag, ctx->nomad_tag_prefix);
            return -1;
        }
        nomad_tag_str = tag + nomad_tag_len;
        flb_plg_info(ctx->ins, "tag for regex (%s)", nomad_tag_str);
        nomad_tag_len = tag_len - nomad_tag_len;

        n = flb_regex_do(ctx->regex, nomad_tag_str, nomad_tag_len, &result);
    }
    ...
    if (meta->jobname && meta->namespace) {
        /* calculate estimated buffer size */
        n = meta->namespace_len + 1 + meta->jobname_len + 1;
        if (meta->task_name) {
            n += meta->task_name_len + 1;
        }
        ...
        /* Copy jobname */
        memcpy(meta->cache_key + off, meta->jobname, meta->jobname_len);
        off += meta->jobname_len;

        if (meta->task_name) {
            /* Separator */
            meta->cache_key[off++] = ':';
            memcpy(meta->cache_key + off, meta->task_name, meta->task_name_len);
            off += meta->task_name_len;
        }
...
/*
 * Given a fixed meta data (namespace and jobname), get API server information
 * and merge buffers.
 */
static int get_and_merge_meta(struct flb_nomad *ctx, struct flb_nomad_meta *meta,
                              char **out_buf, size_t *out_size)
{
    int ret;
    char *api_buf;
    ...
    //     ret = get_pods_from_kubelet(ctx, meta->namespace, meta->jobname,
    //                                 &api_buf, &api_size);
    // }
    else {
        ret = get_api_server_info(ctx, meta->namespace, meta->jobname,
                                  &api_buf, &api_size);
    }
    if (ret == -1) {
        return -1;
 ...
 * for DNS to start up.
 */
static int wait_for_dns(struct flb_nomad *ctx)
{
    int i;
    struct addrinfo *res;
...
static int flb_nomad_network_init(struct flb_nomad *ctx, struct flb_config *config)
{
    ...
    if (ctx->api_https == FLB_TRUE) {
        if (!ctx->tls_ca_path && !ctx->tls_ca_file) {
            ctx->tls_ca_file  = flb_strdup(FLB_NOMAD_CA);
        }
...
int flb_nomad_meta_init(struct flb_nomad *ctx, struct flb_config *config)
{
    ...
    /* Init network */
    flb_nomad_network_init(ctx, config);
    ...
    if (ret == FLB_TRUE && !ctx->use_tag_for_meta) {
        ...
        //     ret = get_pods_from_kubelet(ctx, ctx->namespace, ctx->jobname,
        //                                       &meta_buf, &meta_size);
        // }
        // else {
            /* Gather info from API server */
            flb_plg_info(ctx->ins, "testing connectivity with API server...");
            ret = get_api_server_info(ctx, ctx->namespace, ctx->jobname,
                                   &meta_buf, &meta_size);
     //   }
        if (ret == -1) {
            if (!ctx->jobname) {
                      ...
                      struct flb_nomad_props *props)
{
    ...
    flb_plg_warn(ctx->ins, "meta f: %d", ret);
    flb_plg_warn(ctx->ins, "meta m: %s - %s %d", tag, data, data_size);
    ...
    if (ret == -1) {
        ...
        flb_plg_warn(ctx->ins, "meta merge: %d", ret);
        if (ret == -1) {
            *out_buf = NULL;
            *out_size = 0;
            return 0;
        ...
        id = flb_hash_add(ctx->hash_table,
                          meta->cache_key, meta->cache_key_len,
                          tmp_hash_meta_buf, hash_meta_size);
        flb_plg_warn(ctx->ins, "meta id: %d", id);
        if (id >= 0) {
            /*
...
flb_plg_warn(ctx->ins, "meta res: %s - %d", hash_meta_buf, hash_meta_size);
    ...
    *out_size = off;
    ...
    flb_plg_warn(ctx->ins, "unpack ret: %d", ret);
    if (ret == MSGPACK_UNPACK_SUCCESS) {
        flb_plg_warn(ctx->ins, "meta prop: %s", props);
        /* Unpack the remaining data into properties structure */
        flb_nomad_prop_unpack(props,
...
int flb_nomad_meta_release(struct flb_nomad_meta *meta)
{
    ...
    if (meta->jobname) {
        flb_free(meta->jobname);
        r++;
    }

    if (meta->task_name) {
        flb_free(meta->task_name);
        r++;
    }
